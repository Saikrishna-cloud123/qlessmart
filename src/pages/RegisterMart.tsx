...
import {
  Store, ArrowLeft, Plus, MapPin, Upload, CheckCircle2,
} from 'lucide-react';
...
const branchSchema = z.object({
  branch_name: z.string().trim().min(2, 'Branch name must be at least 2 characters').max(100, 'Branch name too long'),
  inventory_api_url: z.string().url('Must be a valid URL').or(z.literal('')).optional(),
...
const configSchema = z.object({
  ...
  customer_pay_from_app: z.boolean(),
});
...
const RegisterMart = () => {
  ...
  const addBranch = async () => {
    const result = branchSchema.safeParse({
      ...
      inventory_api_url: branchUrl || undefined,
    ...
    const { error } = await supabase.from('branches').insert({
      ...
      inventory_api_url: result.data.inventory_api_url || null,
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-card p-8">
            ...
            <form onSubmit={(e) => { e.preventDefault(); addBranch(); }} className="space-y-4">
              ...
              <div>
                ...
                <Input
                  placeholder="https://api.yourmart.com/product/{barcode}"
                  value={branchUrl}
                  onChange={(e) => setBranchUrl(e.target.value)}
                  className="font-mono text-sm"
                  maxLength={500}
                />
                {errors.inventory_api_url && <p className="mt-1 text-xs text-destructive">{errors.inventory_api_url}</p>}
                <p className="mt-1 text-xs text-muted-foreground">Leave empty to use mock product data</p>
              </div>
              <div>
        ...
        {step === 'config' && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-card p-8">
            <div className="mb-6 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Set up payment options for your store</p>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); saveConfig(); }} className="space-y-4">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-foreground">Merchant Name</label>
                <Input
              ...
              <label className="flex items-center gap-3 cursor-pointer rounded-xl border border-border p-4">
                <input
                  type="checkbox"
                  checked={payFromApp}
                ...
                />
                <div>
                  <span className="text-sm font-medium text-foreground">Allow in-app payments</span>
                  <p className="text-xs text-muted-foreground">Let customers pay via UPI/Razorpay from the app</p>
                </div>
              </label>
              <div className="flex gap-3 pt-2">
